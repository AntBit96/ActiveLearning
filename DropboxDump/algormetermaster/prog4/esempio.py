...
for K in [1]:
    ...
    X_notm = np.empty(( 0 , np.size(Xpts,1) ))
    X_m = np.empty(( 0 , np.size(Xpts,1) ))
    ...
    for i in range(np.size(Xpts,0) ):
     ...
     if i != imid1 and i != imid2:
      for j in range(np.size(Xpts,1)):
       new_e[0,j] = Xpts[i,j] 
      X_notm = np.vstack( (X_notm, new_e) )
      y_notm = np.append(y_notm, [ylbs[i]], axis=0)
     else:
      for j in range(np.size(Xpts,1)):
       new_e[0,j] = Xpts[i,j]
      X_m = np.vstack( (X_m, new_e) )
      y_m = np.append(y_m, [ylbs[i]], axis=0)
    ...
    plt.contourf(xx, yy, y_pred_shaped, levels=1, cmap='Greys_r' , alpha = 0.2)
      
    XDuplex = np.vstack( (X_notm, X_m ) )
    L1 = np.append(y_notm, y_m , axis=0)
    ...
    for i in range(np.size(X_m,0) ):
     ...
     if( int( y_m[ i ] ) == -1 ):
      for j in range(np.size(X_m,1)):
       ...
       XDuplex[i + np.size(X_notm,0) ,j] = X_m[i,j] + x[ offset + j ]
    ...
    print ( 'new x  = ' , XDuplex , file=f_out)
    ...
    plt.contourf(xx, yy, y_pred_shaped, levels=1, cmap = plt.cm.Paired, alpha = 0.99)

    #plt.scatter(XDuplex[:, 0], XDuplex[:, 1], c = L1, cmap = plt.cm.Paired)

    plt.scatter(XDuplex[L1 == -1, 0], XDuplex[L1 == -1, 1], c = 'red', s = 70, marker='_')
    plt.scatter(XDuplex[L1 == 1, 0], XDuplex[L1 == 1, 1], c = 'red' , s = 70, marker='+')  
